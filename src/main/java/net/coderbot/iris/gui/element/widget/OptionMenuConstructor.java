...
public final class OptionMenuConstructor {
    ...
    static {
        registerScreen(OptionMenuMainElementScreen.class, screen ->
                new ElementWidgetScreenData(new TextComponentString(Iris.getCurrentPackName()).appendText(Iris.isFallback() ? " (fallback)" : "").withStyle(TextFormatting.BOLD), false));

        registerScreen(OptionMenuSubElementScreen.class, screen ->
                new ElementWidgetScreenData(GuiUtil.translateOrDefault(new TextComponentString(screen.screenId), "screen." + screen.screenId), true));
